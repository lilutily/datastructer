...
public class MergeSort {
    // 병합 작업에 쓰일 임시 배열
    private static int[] temp; // 짤라둔 배열을 담아둘 임시 배열
    ...
    private static void mergeSort(int[] arr, int s, int e) {
        ...
        // 분할의 중앙점을 찾아야 함
        int m = (s + e) / 2; // ex) s=0, e=7 이면 m=3

        // 분할 작업
        mergeSort(arr, s, m); // 0 ~ 3
        mergeSort(arr, m + 1, e); // 4 ~ 7

        // 병합 작업

        // 부분 배열을 임시 배열에 저장
        for (int i = s; i <= e; i++) { // 0 ~ 3 까지의 배열을 임시배열에 옮겨둠
            temp[i] = arr[i];
        }
    ...
    public static void sort(int[] arr) {
        ...
        mergeSort(arr, 0, arr.length - 1); // 정렬할 배열, 시작위치, 끝위치
    }
