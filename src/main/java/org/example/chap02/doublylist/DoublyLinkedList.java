...
class Node {
    ...
    public class DoublyLinkedList {
        ...
        public void add(int data) {
            ...
            while (current != null && data > current.getData()) { // 정렬된 리스트니까 삽입할 노드가 더 크면 이동을 계속한다
                prev = current;
            ...
            // 새로운 노드가 맨 처음 위치에 삽입되는 경우
                ...
                current.setPrevNode(newNode); // 새로 들어온 노드와 연결(양방향이니까)
                newNode.setNextNode(header.getFirstNode());  // 맨 앞으로 왔으니까 기존에 있던 첫번째 노드를 봐야됨
                header.setFirstNode(newNode); // header 변경
            }
            // 새로운 노드가 맨 마지막 위치에 삽입되는 경우
            else if (current == null) {
                prev.setNextNode(newNode); // 기존에 있던 마지막 노드를 새로 들어온 마지막 노드에 연결
                newNode.setPrevNode(prev); // 새로운 노드를 앞에 노드와 연결
                header.setLastNode(newNode);
            }
